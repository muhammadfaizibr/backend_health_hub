...
class Profile(models.Model):
    ...
    category = models.CharField(
        max_length=100, 
        choices=DOCTOR_CATEGORIES,
        default='general_practitioner',
        db_index=True,
        help_text='Primary medical category/specialty'
    )
    specialization = models.CharField(
        max_length=255, 
        blank=True,
        help_text='Detailed specialization or sub-specialty'
    )
    years_of_experience = models.PositiveIntegerField(default=0, db_index=True)
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['category', 'years_of_experience']),
        ]
        ordering = ['-created_at']

    def __str__(self):
        return f"Dr. {self.user.get_full_name()} - {self.get_category_display()}"
    ...
    def get_category_label(self):
        """Get the display label for category."""
        return dict(DOCTOR_CATEGORIES).get(self.category, self.category)
